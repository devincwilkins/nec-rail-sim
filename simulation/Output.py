...
def outputFigs(out_path, vehicleInfo, stop_df):
    ...
    columns=['time','id','route_id','route_name','state','speed','goal_speed','track', \
                'speed_limit','location_id','location_distance', 'track_distance','total_distance', 'master_block','sub_block','route_direction','signal'])
    ...
    def output_multitrack():
        ...
        for i,track in enumerate(unique_tracks): 
            ...
            # print(stop_df)
            aux_stop_df = stop_df.query(f"Track_1.astype('str')=='{track}'")
            ...
            aux_output_df = output_vehicle.sort_values(['id','time'],ascending=True).query(f"track_distance!=0 and track=='{track}'")
            ...
            aux_ax.set_yticks(aux_stop_df.loc[:,'East_bound_1'].sort_values(ascending=True))
            aux_ax.set_yticklabels(aux_stop_df.sort_values('East_bound_1',ascending=True).Stop_name, fontsize = 9) ## .map(lambda x: x.rstrip('_5'))
        ...
        plt.tight_layout()
        plt.savefig(out_path + 'runGraphMulti.png')

    def output_onetrack():
        #update to display station locations on y-axis 
        NCOLS = 1
        NROWS = 1
        DICT_COLORS = { route_name:f'C{i}' for i,route_name in enumerate(output_vehicle.route_name.unique())}

        fig, ax = plt.subplots(figsize=(20,10*NROWS), ncols=NCOLS,nrows=NROWS, sharex=True, sharey=False)

        aux_output_df = output_vehicle.sort_values(['id','time'],ascending=True).query(f"track_distance!=0")
        for name, group in aux_output_df.groupby('track_route_segment'): 
            ax.plot(group.time,group.total_distance, color=DICT_COLORS.get(group.route_name.iloc[0]))
        ax.set_xlabel('Time', fontsize = 10)
        ax.set_ylabel('Location', fontsize = 10)
        ax.yaxis.grid(True)
        ax.set_ylim(0,780000)
        ...
        if num_tracks > NCOLS:
            ...
            ax.legend(handles,labels,bbox_to_anchor=(1.0,1.1))
        plt.tight_layout()
        plt.savefig(out_path + 'runGraphOne.png')
